...
def generate_input(K_min, K_max, N_K, M, channel, cross_section, profile, V_conv, L, K_diff_0, delta):
    ...
    K = K.reshape(-1, 1)
    y = y.reshape(-1, 1)
    parameter = delta*np.ones(N_K).reshape(-1, 1)
...
model = rnn_model()
X_scaled = X_scaled.reshape(N_delta, N_K, 2)
y_scaled = y_scaled.reshape(N_delta, N_K, 1)
...
pred_time = []
for c in range(20):
    delta_ = np.random.uniform(delta_min, delta_max)
    start = timer()
    X, y = generate_input(K_min, K_max, N_K, M, channel, cross_section,
                          profile, V_conv[i], L[i], K_diff_0[i], delta_)
    end = timer()
...
delta_ = [0.5, 0.6, 0.7, 0.8]
for c, d in enumerate(delta_):
    X, y = generate_input(K_min, K_max, N_K, M, channel,
                          cross_section, profile, V_conv[i], L[i], K_diff_0[i], d)
    ...
    ax.plot(np.power(10, X[:, 0]), np.power(
        10, y), color=color[c], label=(r'$\delta$' + ' = {}'.format(d)))
...
fig.tight_layout()
plt.savefig('figures/rnn_delta.pdf')
